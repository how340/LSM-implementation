...
LSM_Tree::LSM_Tree(float FPR, size_t level_ratio, size_t buffer_size)
    : FPR(FPR), level_ratio(level_ratio), buffer_size(buffer_size)
{
...
/**
 * LSM_Tree create_run()
 * This function creates a new run to insert at a level. The MONKEY optimization
 * is implemented here by dynamically implementing bits per_entry base on the
 * floating point false positive rate specified at lsm tree instantiation.
 *
 * @param  {std::vector<Entry_t>} buffer : a buffer that contains the
 * @return {Run}                         : a run object containing memory address
 *                                         of fence pointer, bloom filter, and string
 *                                         of the file address on disk.
 */
...
void LSM_Tree::save_to_memory(std::string filename, std::vector<KEY_t> *fence_pointer, std::vector<Entry_t> &vec)
{
    ...
    { // go through all entries.
        ...
        // fence_pointer is nullptr if used by exist_save_memory().
        if (memory_cnt == 0 && fence_pointer)
        {
...
void LSM_Tree::exit_save_memory()
{
    ...
    save_to_memory(filename, nullptr, buffer);
}
...
void LSM_Tree::level_meta_save()
{
    ...
    meta << FPR << " " << level_ratio << " " << buffer_size << "\n";
