...
what to save delete as. (add an additional bit?) -- pretty much complete now.
2. Implement a more advanced merge policy to improve performance.
3. Improve the two optimization proposed in the paper - MONKEY should be complete.
...
LSM_Tree *meta_load_save()
{
    ...
    LSM_Tree *lsm_tree = new LSM_Tree(FPR, level_ratio, buffer_size);
